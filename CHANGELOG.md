...
- The service now writes one structured record per request — correlation id,
  method, route, status, duration, and the redacted device principal when one
  authenticated — and `wuwaterm-api serve` installs the handler that emits it
  (`WUWATERM_API_LOG_LEVEL`, default `INFO`). It had none: every `LOGGER` call
  in the adapter went nowhere in a deployed process, so a request id handed to
  a client matched nothing on the server. The operator subcommands and any
  program importing the application still configure no logging at all.
