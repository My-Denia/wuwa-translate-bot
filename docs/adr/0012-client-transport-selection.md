...
| Verification cost used as the load | Non-queuing admission sheds with `429`; a dedicated, bounded credential pool is the real ceiling and cannot be released by a caller | No total in-flight ceiling across all routes; see the residual risks in the architecture map |
