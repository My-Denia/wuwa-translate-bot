...
`Ctrl+1/2/3` 与 `Ctrl+K` 键盘跳区、未配置态三步引导清单、句子短路直达翻译区。

> 本行原先还列了「请求刹车与 LRU 缓存」。它们连同输入即搜整体**未实现**——
> 在本 PR 的中间提交里存在过,合并前撤掉,查词维持提交驱动。撤掉的理由与
> 重新实现前必须先解决的判定点见配套 issue;交付行为见
> `docs/client-ui-redesign.md` §4.1。此处保留记录而不删行,是因为这条吸收
> 承诺是当初判定「保留三区」胜出的论据之一,读者有权看到它后来只兑现了一部分。
