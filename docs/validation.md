...
| `tests/test_client_documentation_claims.py` | The client's own documentation does not promise properties the client does not have: no file promises that two builds produce the same bytes, the build script states the narrower guarantee it does provide, and the README describes cancellation as it behaves on both sides. A text gate, for defects that are sentences rather than code | The repository `pytest` run, beside the other text gates |
