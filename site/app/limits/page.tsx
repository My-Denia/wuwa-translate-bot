...
export default function Limits() {
  return <main className="document"><Link className="back-link" href="/">← 返回 WuwaTerm</Link><p className="eyebrow">SHARED BETA POOL</p><h1>大家共用一份额度。</h1><p className="document-lead">一个访客可能用完整个共享池。我们限制总使用量，不保证每个人都能分到额度。</p>
    <section><h2>当前公测额度</h2><table><thead><tr><th>功能</th><th>全站上限</th></tr></thead><tbody><tr><td>术语查询</td><td>{limits.termsPerDay} 次 / UTC 日</td></tr><tr><td>整句翻译</td><td>{limits.translationsPerDay} 次，且输入合计 {limits.charactersPerDay.toLocaleString('zh-CN')} 字符 / UTC 日</td></tr><tr><td>译文审校</td><td>{limits.reviewsPerDay} 次 / UTC 日</td></tr><tr><td>所有服务请求</td><td>每自然分钟 {limits.upstreamPerMinute} 次，每秒最多 1 次</td></tr><tr><td>整句翻译短窗口</td><td>每自然分钟 {limits.translationsPerMinute} 次</td></tr></tbody></table><p>每日额度于 UTC 00:00（北京时间 08:00）重置。术语输入最多 200 字符，整句和审校原文/译文各最多 2,000 字符。整句翻译可能暂未开放，以首页状态为准。</p></section>
    <section><h2>翻译用完了，仍可查术语</h2><p>查询池与翻译池相互独立。翻译关闭或日额度用尽，不会扣减或关闭查询池。查询仍受自身日额度、全站短窗口和服务可用性限制。</p></section>
    <section><h2>稿件续作与重新核对</h2><p>保存、导入、选择词对、编辑、比较和导出都在本地完成，不消耗服务请求。每次主动“核对术语”或“仅获取新依据”各使用一次审校请求，不消耗翻译次数或翻译字符额度。导入后先获取新依据，恢复的适用选择需要再主动核对一次；不会自动重试。</p><p>稿件文件上限为 1 MiB，原文和译文各最多 2,000 字符，最多保留 32 处选择、64 个对应范围及最近两份报告。文本编辑后，已确认的分段对应需要重新确认。无法比较的发现不会被标记为问题已解决。</p></section>
    ...
    <footer><a href="/privacy">隐私说明</a> · 公测额度更新：2026-09-07</footer>
  </main>;
}
